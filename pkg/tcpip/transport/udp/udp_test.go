...
func buildV4Packet(payload []byte, addresses context.Header4Tuple, badChecksum bool) buffer.View {
	...
	ip.Encode(&header.IPv4Fields{
		...
		SrcAddr:     addresses.SrcAddr.Addr,
		DstAddr:     addresses.DstAddr.Addr,
	})
	...
	u.Encode(&header.UDPFields{
		SrcPort: addresses.SrcAddr.Port,
		DstPort: addresses.DstAddr.Port,
		Length:  uint16(header.UDPMinimumSize + len(payload)),
	})

	// Calculate the UDP pseudo-header checksum.
	xsum := header.PseudoHeaderChecksum(udp.ProtocolNumber, addresses.SrcAddr.Addr, addresses.DstAddr.Addr, uint16(len(u)))
...
func buildV6Packet(payload []byte, addresses context.Header4Tuple, badChecksum bool) buffer.View {
	// Allocate a buffer for data and headers.
...
func injectPacket(c *context.Context, flow context.TestFlow, payload []byte, badChecksum bool) {
	var buf buffer.View
	addresses := flow.MakeHeader4Tuple(context.Incoming)
	if flow.IsV4() {
		buf = buildV4Packet(payload, addresses, badChecksum)
	} else {
		buf = buildV6Packet(payload, addresses, badChecksum)
	}
	...
	defer pkt.DecRef()
	c.LinkEP.InjectInbound(flow.NetProto(), pkt)
}
...
func testReadInternal(c *context.Context, flow context.TestFlow, packetShouldBeDropped, expectReadError bool, checkers ...checker.ControlMessagesChecker) {
	c.T.Helper()

	payload := newPayload()
	injectPacket(c, flow, payload, false)
...
func TestV4UnknownDestination(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("flow:%s icmpRequired:%t largePayload:%t badChecksum:%t", tc.flow, tc.icmpRequired, tc.largePayload, tc.badChecksum), func(t *testing.T) {
			...
			injectPacket(c, tc.flow, payload, tc.badChecksum)
			if tc.badChecksum {
				checksumErrors++
...
func TestBadChecksumErrors(t *testing.T) {
	for _, flow := range []context.TestFlow{context.UnicastV4, context.UnicastV6} {
		t.Run(flow.String(), func(t *testing.T) {
			...
			payload := newPayload()
			injectPacket(c, flow, payload, true /* badChecksum */)
