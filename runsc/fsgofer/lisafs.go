...
func newControlFDLisa(c *lisafs.Connection, hostFD int, parent *controlFDLisa, name string, mode linux.FileMode) *controlFDLisa {
	var (
	...
	childFD.ControlFD.Init(c, childNode, mode, childFD)
	return childFD
}
...
func (fd *controlFDLisa) Walk(c *lisafs.Connection, name string) (*lisafs.ControlFD, linux.Statx, error) {
	...
	return newControlFDLisa(c, childHostFD, fd, name, linux.FileMode(stat.Mode)).FD(), stat, nil
}
...
func (fd *controlFDLisa) Open(c *lisafs.Connection, flags uint32) (*lisafs.OpenFD, int, error) {
	flags |= openFlags
...
func (fd *controlFDLisa) OpenCreate(c *lisafs.Connection, mode linux.FileMode, uid lisafs.UID, gid lisafs.GID, name string, flags uint32) (*lisafs.ControlFD, linux.Statx, *lisafs.OpenFD, int, error) {
	...
	childFD := newControlFDLisa(c, childHostFD, fd, name, linux.ModeRegular)
...
func (fd *controlFDLisa) Mkdir(c *lisafs.Connection, mode linux.FileMode, uid lisafs.UID, gid lisafs.GID, name string) (*lisafs.ControlFD, linux.Statx, error) {
	...
	cu.Release()
	return newControlFDLisa(c, childDirFd, fd, name, linux.ModeDirectory).FD(), childDirStat, nil
}

// Mknod implements lisafs.ControlFDImpl.Mknod.
func (fd *controlFDLisa) Mknod(c *lisafs.Connection, mode linux.FileMode, uid lisafs.UID, gid lisafs.GID, name string, minor uint32, major uint32) (*lisafs.ControlFD, linux.Statx, error) {
	...
	return newControlFDLisa(c, childFD, fd, name, mode).FD(), childStat, nil
}

// Symlink implements lisafs.ControlFDImpl.Symlink.
func (fd *controlFDLisa) Symlink(c *lisafs.Connection, name string, target string, uid lisafs.UID, gid lisafs.GID) (*lisafs.ControlFD, linux.Statx, error) {
	...
	cu.Release()
	return newControlFDLisa(c, symlinkFD, fd, name, linux.ModeSymlink).FD(), symlinkStat, nil
}

// Link implements lisafs.ControlFDImpl.Link.
func (fd *controlFDLisa) Link(c *lisafs.Connection, dir lisafs.ControlFDImpl, name string) (*lisafs.ControlFD, linux.Statx, error) {
	...
	cu.Release()
	return newControlFDLisa(c, linkFD, dirFD, name, linux.FileMode(linkStat.Mode)).FD(), linkStat, nil
}
...
func (fd *controlFDLisa) Connect(c *lisafs.Connection, sockType uint32) (int, error) {
	if !c.ServerImpl().(*LisafsServer).config.HostUDS {
...
func (fd *controlFDLisa) RenameAt(c *lisafs.Connection, oldName string, newDir lisafs.ControlFDImpl, newName string) error {
	return renameat(fd.hostFD, oldName, newDir.(*controlFDLisa).hostFD, newName)
}

// Renamed implements lisafs.ControlFDImpl.Renamed.
func (fd *controlFDLisa) Renamed(c *lisafs.Connection) {
...
func (fd *openFDLisa) Read(c *lisafs.Connection, buf []byte, off uint64) (uint64, error) {
	rw := rwfd.NewReadWriter(fd.hostFD)
...
func (fd *openFDLisa) Renamed(c *lisafs.Connection) {
